...
const HERO_SLOT_SCENE := preload("res://scenes/ui/hero_slot.tscn")
const SKILL_SLOT_SCENE := preload("res://scenes/ui/skill_slot.tscn")
const EQUIPMENT_SLOT_SCENE := preload("res://scenes/ui/equipment_slot.tscn")
const SPECIAL_EQUIPMENT_SLOT_SCENE := preload("res://scenes/ui/special_equipment_slot.tscn")
const CURRENT_HERO_CONTENT_SCENE := preload("res://scenes/ui/current_hero_content.tscn")
const EQUIPMENT_CONTENT_SCENE := preload("res://scenes/ui/equipment_content.tscn")
...
func _bind_authored_scene() -> void:
	var root := get_node_or_null("HUDRoot") as Control
	var combat_host := get_node_or_null("HUDRoot/CombatViewportContainer") as SubViewportContainer
	var bottom := get_node_or_null("HUDRoot/BottomHUD") as Control
	var debug := get_node_or_null("HUDRoot/DebugDrawer") as VBoxContainer
	if root == null or combat_host == null or bottom == null or debug == null:
		push_error("BattleHUD requires authored HUDRoot, CombatViewportContainer, BottomHUD and DebugDrawer")
		set_process(false)
		return
	_combat_viewport = combat_host.get_node_or_null("CombatViewport") as SubViewport
	_proxy_camera = combat_host.get_node_or_null("CombatViewport/CombatCamera") as Camera3D
	var team_panel := bottom.get_node_or_null("BottomBody/ABCColumns/TeamPanel") as PanelContainer
	var hero_panel := bottom.get_node_or_null("BottomBody/ABCColumns/CurrentHeroPanel") as PanelContainer
	var equip_panel := bottom.get_node_or_null("BottomBody/ABCColumns/EquipmentPanel") as PanelContainer
	if _combat_viewport == null or _proxy_camera == null or team_panel == null or hero_panel == null or equip_panel == null:
		push_error("BattleHUD requires authored viewport, camera and three section panels")
		set_process(false)
		return
...
func _build_team_slots(parent: VBoxContainer) -> void:
	_team_slot_row = parent.get_node("HeroSlots") as HBoxContainer


func _make_team_slot(hero: Node3D, index: int, seat: Control) -> PanelContainer:
	var frame := HERO_SLOT_SCENE.instantiate() as AspectRatioContainer
	frame.name = "HeroSlotFrame%d" % (index + 1)
	seat.add_child(frame)
	var slot := frame.get_node("HeroSlot") as PanelContainer
	slot.name = "HeroSlot%d" % (index + 1)
	slot.gui_input.connect(_on_team_slot_gui_input.bind(index))
	var inner := slot.get_node("Inner") as VBoxContainer
	(inner.get_node("Top/Index") as Label).text = "%02d" % (index + 1)
	(inner.get_node("Top/Hotkey") as Label).text = "F%d" % (index + 1)
	(inner.get_node("HeroAvatar") as TextureRect).texture = _hero_portrait(hero)
	(inner.get_node("HeroName") as Label).text = _hero_display_name(hero)
	(inner.get_node("HeroHpTrack/MiniHpFill") as ColorRect).anchor_right = _health_ratio(hero)
	return slot


func _build_current_hero(parent: VBoxContainer) -> void:
	var content := CURRENT_HERO_CONTENT_SCENE.instantiate() as HBoxContainer
	parent.add_child(content)
	_portrait_texture = content.get_node("HeroPortraitFrame/HeroPortrait/PortraitTexture") as TextureRect
	_portrait_fallback_label = content.get_node("HeroPortraitFrame/HeroPortrait/PortraitFallback") as Label
	var info := content.get_node("NameLevel") as VBoxContainer
	_name_label = info.get_node("NameRow/HeroName") as Label
	_level_label = info.get_node("NameRow/HeroLevel") as Label
	_control_mode_label = info.get_node("NameRow/ControlMode") as Label
	_role_label = info.get_node("HeroRole") as Label
	_hp_fill = info.get_node("HPBar/Fill") as ColorRect
	_hp_text = info.get_node("HPBar/Label") as Label
	_mp_fill = info.get_node("MPBar/Fill") as ColorRect
	_mp_text = info.get_node("MPBar/Label") as Label
	var skills := content.get_node("SkillContainer") as HBoxContainer
	_skill_slots.clear()
	_skill_icons.clear()
	_skill_glyphs.clear()
	...
	for index in SKILL_KEYS.size():
		...
		var frame := SKILL_SLOT_SCENE.instantiate() as AspectRatioContainer
		frame.name = "SkillFrame%s" % key
		skills.add_child(frame)
		var slot := frame.get_node("SkillSlot") as PanelContainer
		slot.name = "SkillSlot%s" % key
		var stack := slot.get_node("Stack") as Control
		var icon := stack.get_node("SkillIcon") as TextureRect
		var glyph := stack.get_node("SkillGlyph") as Label
		glyph.text = key
		(stack.get_node("KeyBadge") as Label).text = key
		_skill_icons.append(icon)
		_skill_glyphs.append(glyph)
		_skill_cooldown_masks.append(stack.get_node("CooldownMask") as ColorRect)
		_skill_cooldown_labels.append(stack.get_node("CooldownLabel") as Label)
		_skill_slots.append(slot)


func _build_equipment(parent: VBoxContainer) -> void:
	var content := EQUIPMENT_CONTENT_SCENE.instantiate() as HBoxContainer
	parent.add_child(content)
	var row1 := content.get_node("EquipCluster/EquipRow1") as HBoxContainer
	var row2 := content.get_node("EquipCluster/EquipRow2") as HBoxContainer
	for index in 3:
...
func _make_equipment_slot(index: int, glyph: String, key_text: String, is_special: bool) -> AspectRatioContainer:
	var frame := (SPECIAL_EQUIPMENT_SLOT_SCENE if is_special else EQUIPMENT_SLOT_SCENE).instantiate() as AspectRatioContainer
	frame.name = "SpecialFrame" if is_special else ("EquipFrame%d" % index)
	var slot := frame.get_node("EquipmentSlot") as PanelContainer
	slot.name = "SpecialEquipmentSlot" if is_special else ("EquipmentSlot%d" % index)
	(slot.get_node("Glyph") as Label).text = glyph
	(slot.get_node("Hotkey") as Label).text = key_text
	return frame
