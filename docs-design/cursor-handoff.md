...
- 阵营识别改用脚底软色环（蓝/红，低透明），叠在 GroundShadow 外圈；不要恢复像素 Outline Glow。霸体轮廓仍保留。
...
- 主训练场：`DNF_Style_Prototype.tscn`；瑞兹：`scenes/units/ryze.tscn`；AI/技能：`scripts/characters/ryze_actor.gd`；战斗 HUD：`scripts/ui/battle_hud.gd`（底部 A/B/C + Debug）；训练场编队：`scripts/ui/training_roster_panel.gd`（嵌在 Debug → Training Tools）。
...
T：taunt 动画为觉醒；引导期间霸体，结束后 6s T_Buff；完整通用法术吸血机制，保留被动 10/20/30% 冷却缩减；外溢半径 3.5m。Cut-In 走 `AwakeningCutIn` autoload，素材在 `assets/presentation/awakening/ryze/`。

R：2s 引导，仅沉默/眩晕/击飞打断，取消返还冷却；传送瑞兹及周围 5.5m 同队友军（共用平面位移、各自钳边界），最远 25m；落地播放角色 spell4_winddown 和 Spell4_R_winddown，对周围 5.5m 每个敌人直接三次 E 初段伤害与三层减抗，不生成 E 弹道/弹射。
