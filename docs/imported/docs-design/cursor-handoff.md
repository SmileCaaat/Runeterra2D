...
3. 普攻、Q、E、impact：用户期望单帧画布中心对应特效锚点；CastAnchor 对应出手点，HitAnchor 对应目标命中点。W、W_loop、R winddown 走脚底地面锚点。角色脚底定位与飞行特效中心是不同契约。
...
- 角色 `_configure_character_anchor` 一类定位逻辑读取角色 JSON 原点；不要因为飞行特效采用画布中心而删除角色脚底定位。
