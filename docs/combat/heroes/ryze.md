...
> 本文是瑞兹的唯一档案。前半有早期设计快照，其中“QWE 全程尾三帧取消”、护盾滤色及“尚未实现/未入表”等表述已过时，以本文后半锁定决策和文末视觉验收为准。已有 CSV、场景和脚本，不要据此从零重做；已有文件也不等于功能验收完成。普攻第 6 帧发球、Q 第 4 帧发球、W 第 6 帧结算、E 第 7 帧发球（表内零基 5/3/5/6）；普攻和 QE 在投射物接触时结算。超负荷改为施法加速 60% 加盖伦 Q 残影，不再压尾帧。T 按爆发窗开，R 按开团/切入/逃生折跃。`TBuff` 与 `RWinddown` 以当前 `ryze.tscn` 为准。工程交接见 [cursor-handoff.md](../../../docs-design/cursor-handoff.md)。
...
- P 对每一次主动技能施放完成叠加 1 层奥术精通；所有层数统一刷新为 6 秒、最多 5 层。T 自身也可以叠层，但绝不消耗超负荷的强化施法次数。
...
- T（绝望之力）是爆发强化 Buff：只在可兑现连招时开（Q/W/E 至少两个就绪，或奥术 ≥4 层，或已在超负荷）。不按自身残血开。
...
- 超负荷窗口要让连招更强势：窗口内普攻与 Q/W/E 的施法动画速度提高 60%（`ryze.supercharge.cast_speed_scale = 1.6`），并复用盖伦破舰 Q 的加速残影。不压缩、不取消尾帧。T、R、受击与跑步不受该加速影响。
...
| Q · Overload | 方向投射物；来源距离 `550 → 5.5m`、宽 `110 → 1.1m`、速度 `1700 → 17m/s`；仅首个命中敌人受击；纵深容差 `1.1m` | 无霸体；伤害有效时点是法球接触目标，而非角色 `spell1` 的某一帧；命中反馈与盖伦普攻相同：停顿 `0.06s`、硬直 `0.24s`、击退初速 `2.8m/s`；超负荷期间普攻与 QWE 施法加速 60% 并带盖伦 Q 残影，不压尾帧 | 角色播放 `spell1`；命中时播放 `impact` |
...
| T · Desperate Power | 自身；基本技能外溢半径 `350 → 3.5m`；来源 `+80` 移速按既定比例换算为 `+0.8m/s` | 播放 `taunt` 后进入 6 秒觉醒强化；引导期间拥有霸体，强化持续期不默认继承霸体；保留被动冷却缩减 `10 / 20 / 30%` | 觉醒 Cut-In 使用原画与 `Ryze_awake.wav`；`TBuff` 使用 `ryze.tscn` 手调坐标 `(-0.52, 5.24, 1.81)`，随朝向翻转 X，并在强化 6 秒内循环 `T_Buff`；引导期复用盖伦 `SuperArmorOutline` 黄红黄轮廓。触发外溢的主目标命中叠一层 `Lightning Chain` one-shot（`vfx_library_lightning_chain`） |
...
- 不取消、不压缩尾帧。Q/W/E 与普攻都播完整段动画。
- 超负荷窗口内，普攻与 Q/W/E 的 `speed_scale` 设为 `1.6`；命中帧序号不变，只是更快到达。
...
| Desperate Power（History 2nd） | T | `adapted` | 使用 6 秒法术吸血、移速与基本技能半额范围外溢 | 作为觉醒技能保留且不消耗超负荷次数；法术吸血和范围外溢需扩展当前伤害管线 |
...
- 已完成：`schema.version=17` 下的单位/技能/Buff/命中/动画/资产行，以及 `ryze.*` 运行时常量。脚本冷却、射程、弹速、超负荷窗口、减抗剩余倍率、T 外溢倍率和 E 弹射/命中高度等表现常量均读表。
...
| Q 法球 | 场景出手点高度水平飞到敌人 XZ | 场景已写 origin offset | 命中高度 = 出手高度 |
