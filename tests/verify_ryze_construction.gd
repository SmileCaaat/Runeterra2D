...
func _init() -> void:
	...
	if scene != null:
		...
		valid = valid and model.has_method("get_semantic_animation_length")
		...
		if model != null and instance.has_method("_bind_self_vfx_anchors") and instance.has_method("_sync_t_buff_presentation"):
		...
		valid = valid and instance.has_method("has_super_armor") and outline_highlight != null
		valid = valid and instance.has_method("_on_character_model_animation_finished")
		# Retreat movement must face the travel vector, not the opponent it is
		# kiting away from; this guards against a backward Run animation.
		model.call("set_facing", Vector3.RIGHT)
		valid = valid and is_equal_approx(model.rotation.y, PI * 0.5)
		model.call("set_facing", Vector3.LEFT)
		valid = valid and is_equal_approx(model.rotation.y, -PI * 0.5)
		instance.set("action_lock", 9.0)
		instance.call("_on_character_model_animation_finished", &"spell4_winddown")
		valid = valid and is_zero_approx(float(instance.get("action_lock")))
